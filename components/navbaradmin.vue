<template>
  <div class="mt-8">
    <v-navigation-drawer permanent app v-if="GET_LOGIN" color="1E1E1E" dark>
      <v-toolbar-title>
        <v-img max-height="80" max-width="300" class="" :src="require('~/assets/mju_logo_FOOTER.png')">
        </v-img>
      </v-toolbar-title>
      <v-divider></v-divider>
      <v-list nav dense v-for="(item, i) in items" :key="i">
        <v-list-item :key="item.title" class="list-item-shadow" link :to="item.to" v-if="!item.nested"
        ...
        <v-list-group v-else no-action :prepend-icon="item.icon">
          ...
          <v-list-item v-for="(item2, index) in item.subitems" :key="index" link :to="item2.to">
            <v-list-item-icon>
